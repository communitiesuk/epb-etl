...
describe "Acceptance::LogitAdapter" do
  context "when logging an event" do
    before do
      stub_request(:post, "https://api.logit.io/v2").to_return(
        body: JSON.generate(message: "Thanks"), status: 202,
      )
    end
    ...
    it "sends the log event to the logit api" do
      ...
      expect(WebMock).to have_requested(:post, "https://api.logit.io/v2").with(
        body:
          JSON.generate(stage: "stage", event: "event", data: { test: true }),
      )
    end
  end
end
